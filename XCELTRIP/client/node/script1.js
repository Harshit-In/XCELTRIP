...
app.use(cors());
// const web3 = new Web3("https://bsc-dataseed1.binance.org/");//main
// const web3 = new Web3("https://data-seed-prebsc-2-s1.binance.org:8545/"); //testnet
...
const contract_address = "0x051a1D0DC129D2CFb8d7294b9c7B588c118e2802";
      ...
      .then((d) => {
        ...
        contract
          .getPastEvents({
            ...
            toBlock: Number(result[0].latest_block) + 500,
          })
          .then(async (events) => {
            let resu = await generateEventQuery(events);
            if (
              parseInt(result[0].latest_block) + 500 <
          ...
          .catch((e) => {
            console.log("Err", e);
            return res.json({status:0,err:e});
          });
      })
      .catch((e) => {
		console.log("Err", e);
...
async function forEverExcute(){
	let i=0;
	conn.query("SELECT * FROM users where added = 0",async function (err,res){
    ...
    try {
      ...
      web3.setProvider(provider);
        ...
        contract.setProvider(provider);
        const gasPrice = await web3.eth.getGasPrice();
        let userexist = await contract.methods.isUserExists(res[i].user).call();
        let refferexist = await contract.methods.isUserExists(res[i].referral).call();
        console.log("user", userexist, refferexist);
        if (!userexist && refferexist) {
          let gas = await contract.methods
            .owner_registration(res[i].user, res[i].referral)
            .estimateGas({ value: 0, from: accounts[0] });
          contract.methods
            .owner_registration(res[i].user, res[i].referral)
            .send({ value: 0, from: accounts[0], gasPrice: gasPrice, gas: gas })
            .then((d) => {
             console.log(d)
             conn.query(`UPDATE users SET added = '1' WHERE user= '${res[i].user}'`,function (err,resu){
              if(err) throw err;
              console.log(resu);
              i++;
              forEverExcute();
         ...
         console.log("user and reffer not exist",res[i].user,res[i].referral,i)
        }
      } catch (e) {
        console.log(e);
      }
